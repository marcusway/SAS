function SAS_doPWELCH_eo(varargin)
...
for j = 1:nargin
    ...
    file = fullfile(subDir,txtFolder,['SAS_', subDir, 'eor.txt']); %determines name of file from subjid

    try
    [data, ~, ~] = get_big_data(file); %import the file
    catch me
        problem = me.message;
        disp(['MatLab is having trouble reading in the text file, SAS_', subDir, 'eor.txt.  Make sure that file is in the subject''s direotory in the textfiles folder']);
        return
    end
    ...
    for region = 1:length(SAS_regions)
        ...
        save(fullfile(subDir,matFolder,[subDir, '_avg_psds'],[subDir,'_PSDeo', SAS_regions{region},'_mean']), 'PSD');
        csvwrite(fullfile(subDir,txtFolder,[subDir, '_avg_psds'],[subDir,'_PSDeo', SAS_regions{region}, '_mean.txt']),PSD);
        ...
        save(fullfile(subDir,matFolder,[subDir '_ind_electrode_psds'],[subDir, '_PSDeo' SAS_regions{region}]), 'Pxx_matrix');
        
        %open a text file for the individual eleotrode psds.  
        outfile = fopen(fullfile(subDir,txtFolder,[subDir '_ind_electrode_psds'],[subDir 'PSDeo' SAS_regions{region} '_ind.txt']),'w');
